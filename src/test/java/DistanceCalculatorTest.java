...
public class DistanceCalculatorTest {
    ...
    public void testGetDistanceBetweenCoordinates() throws Exception {
        ...
        for(int i = 0; i<1000; i++){
            ...
            assertEquals(distanceToItself, 0, DistanceCalculator.getDistanceBetweenCoordinates(lat, lon, lat, lon), almostZeroInKilometers);
        }

        double distance;
        //expected values were acquired from http://www.movable-type.co.uk/scripts/latlong.html
        distance = DistanceCalculator.getDistanceBetweenCoordinates(50.03, 5.42, 58.38, 3.04);
        assertEquals("distance check", 941.1, distance, 0.05);
        distance = DistanceCalculator.getDistanceBetweenCoordinates(0, 0, 90, 0);
        assertEquals("distance check", 10010, distance, 5);
        distance = DistanceCalculator.getDistanceBetweenCoordinates(0, 0, 0, 180);
        assertEquals("distance check", 20020, distance, 5);
        distance = DistanceCalculator.getDistanceBetweenCoordinates(-50.03, 5.42, 58.38, 3.04);
        assertEquals("distance check", 12060, distance, 5);
        distance = DistanceCalculator.getDistanceBetweenCoordinates(50.0764, 19.9658, 19.9658, 3);
        assertEquals("distance check", 3668, distance, 0.5);
        distance = DistanceCalculator.getDistanceBetweenCoordinates(50.0764, 19.9658, 51.0764, 20.9658);
